...
//		var_dump($companyInfo_arr);exit;
		//企业logo
		$companylogoid=$companyInfo_arr["list"]["companyLogo"];
		if($companylogoid){
			$img=M('files')->where('f_id='.$companylogoid)->find();
			$imgurl=$img["f_filepath"].$img["f_filename"];
		}
		$this->assign("imgurl",$imgurl);
		$this->assign("level",$companyInfo_arr["list"]['level']);
		$this->assign("state",$companyInfo_arr["list"]['state']);
		$this->assign("credit",$companyInfo_arr["list"]['credit']);  //诚意保障金
		$this->assign("supervisionUser",$companyInfo_arr["list"]['supervisionUser']);//是否监管
	}
