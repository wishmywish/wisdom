...
//        $this->assign("taskid",I("get.taskid"));
    ...
    function  surveyTaskAdd(){
        ...
        $this->assign('companyID',  cookie("companyId"));
	    $this->assign('mobile',cookie('mobile'));
        $this->assign("tasktype",I("get.tasktype"));
        $this->assign('bigclass', $re);
        $this->assign('smallclass', $sre);
        $this->display();
    }
    //推荐任务新增入口
    function  recomTaskAdd(){
        set_theme();
        $re = A('Api')->getBigC();
        foreach ($re as $key => $val) {
            if($val['f_sys_column_cid']=='Promotion'){
    ...
    function  editprotask(){
        ...
        if($reModi['result']=="000000"){
            $this->assign('reModi', $reModi);
            $this->assign('gold_coin', ($reModi["pro"]["f_eachcoin"]*10));
        }
//        echo var_dump($reModi);
//        exit;
        $tasktypeid=I("get.tasktypeid");
        $this->assign('mobile',cookie('mobile'));
        $this->assign("taskid",I("get.taskid"));
        $this->assign('bigclass', $re);
        $this->assign('smallclass', $sre);
        $this->assign('tasktype', $tasktypeid);
        ...
        }elseif($tasktypeid==4||$tasktypeid==6||$tasktypeid==5){
            //随手
            $this->display("surveyTaskAdd");
        }elseif($tasktypeid==7){
