...
class HbgameController  extends  Controller
{
    ...
    function    getuseraboutlist($pcid){
            ...
            $result["times"]=1-$times<0?0:1-$times;
...
//        exit;
    ...
    function  index(){
        $re = M("playcity_set")->field("f_status")->where("f_id=2")->find();
        //是否下线
...
//        pt($data);
        ...
        $shop_re = $this->getshop();
        $this->assign("shop",$shop_re);
...
//        f_mypcid=".$f_mypcid."&b_fhelp=".$b_fhelp.
        ...
        $usersum=$this->getpwdbylastuser($b_fmypcid);
//        pt($result);
//        exit();
...
//        exit;
    ...
    function zslogin(){
        $arr = I("post.");
        $re = A('Api/Jhttp')->login($arr['mobile'], md5($arr['password']));
        $arr = json_decode($re,true);
//        pt($arr);
//        exit;
        if($arr["resCode"]=="000000"){
            $out_arr['error_code'] = $arr["resCode"];
            $out_arr['error_text'] = "成功";
            $out_arr['ulist'] = $arr["list"];
        }elseif($arr["resCode"]=="100001") {
            $out_arr['error_code'] = $arr["resCode"];
            $out_arr['error_text'] = "密码错误";
        }else{
                $out_arr['error_code'] = $arr["resCode"];
                $out_arr['error_text'] = A("Api/Jhttpcode")->searchCode($arr["resCode"]);

        }
//        pt($out_arr);
//        exit;
        echo A('Api/Fun')->JSON($out_arr);
    }

    //兑换金币
    function exchange(){
        $arr = I("post.");
        $price = (float)$arr['price']*10;
        if((int)$price===0){
            $out_arr['error_code'] = "200000";
            $out_arr['error_text'] = "您的红包不足！";
        }else {
            $map_s["f_mobile"] = $arr['mobile'];
            $map_v["f_totalglodcoin"] = 0;
            $sum = M("playcity")->where($map_s)->save($map_v);
            if ($sum <= 0) {
                $out_arr['error_code'] = "100000";
                $out_arr['error_text'] = "兑换失败";
            } else {
                D('Api/Shopsheet')->add_shopsheet('events', $arr['userid'], 1, 1, $price, 0, 1, 0, 0);
                $out_arr['error_code'] = "000000";
                $out_arr['error_text'] = "兑换成功";
            }
        }
        echo A('Api/Fun')->JSON($out_arr);
    }
...
//    function curl($arr){
//        $url = "localhost".U('Games/conf')."?command=mylist&mobile=".$arr['mobile']."&userid=".$arr['userid'];
////        echo $url;
//        $ch = curl_init($url);
...
//    }
    ...
    private function getshop(){
        ...
        $shop_j = array("LEFT JOIN t_files ON t_files.f_id = t_shoplist.f_fil eid");
        $out_arr = M("shoplist")->where($shop_map)->join($shop_j)->select();
        return $out_arr;
    }

    //查看我的页面
    private function mylist($in_arr){
        //取我的钱
        $out_arr['own_info'] = M("playcity")->where("f_mobile='".$in_arr['mobile']."'")->find();
        //取产品列表
        $shop_map["f_status"] = 1;
        $shop_map["f_lotterystatus"] = 1;
        $shop_j = array("LEFT JOIN t_files ON t_files.f_id = t_shoplist.f_fileid");
        ...
        $f = "s.f_sid,s.f_shopname,s.f_lottery,MAX(t_own_award.f_shopid) as f_shopid";
...
//        $this->out_arr['buy_list'] = M("own_award")->field($f)->join($j)->where($buy_map)->select();
        foreach($shop_re as $key=>$v){
            if($v["f_lottery"]!=0){
                $lottery_map["f_lottery"] = $v["f_lottery"];
                $lottery_map["f_shopid"] = $v["f_sid"];
                $lottery_re = M("own_award")->field("f_userid")->where($lottery_map)->find();
                if($lottery_re=="" || $lottery_re==0){
                    //中奖号码
                    $shop_re[$key]["mobile"] = "";
                }else{
                    //中奖号码
                    $u_re = A("Api/Jhttp")->getuserinfo2($lottery_re["f_userid"]);
                    $arr = json_decode($u_re,true);
                    $shop_re[$key]["mobile"] = substr_replace($arr["list"]["mobile"],'****',3,4);
                }
            }else{
                $shop_re[$key]["mobile"] = "";
            }
