...
<div class="content">
    <table border="0" cellpadding="0" cellspacing="0" id="clientList">
        ...
        <?php if(is_array($customerClientList['list'])): foreach($customerClientList['list'] as $key=>$vo): ?><tr style="height: 25px;">
            <td>
                <input type="radio" name="more" value="<?php echo ($vo['clientId']); ?>" id="client<?php echo ($vo[clientId]); ?>" <?php if($vo[clientId] == $_GET[clientId]): ?>checked<?php endif; ?>  />
                <label for="client<?php echo ($vo[clientId]); ?>"><?php echo ($vo['clientName']); ?></label>
            </td>
...
<script>
    ...
    var selected_clientId = '<?php echo ($_GET[clientId]); ?>'; //选中的clientId,初始化值
    ...
    $(".search").on("click",function(){
        ...
        $.post("<?php echo U('Office/searchCusClient_ajax');?>", {"page":1, "pageSize":pageSize, "cusId":cusId, "clientSubject":keywords}, function(data) {
    ...
    $("#last_page").on("click",function(){
        ...
        $.post("<?php echo U('Office/searchCusClient_ajax');?>", {"page":totalPage, "pageSize":pageSize, "cusId":cusId, "clientSubject":keywords}, function(data) {
    ...
    $("#prev_page").on("click",function(){
        ...
        $.post("<?php echo U('Office/searchCusClient_ajax');?>", {"page":currentPage-1, "pageSize":pageSize, "cusId":cusId, "clientSubject":keywords}, function(data) {
