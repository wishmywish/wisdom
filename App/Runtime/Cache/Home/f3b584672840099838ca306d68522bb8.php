...
<div class="main">
    ...
    <ul>
        ...
        <li style="position: relative">
            <input type="text" id="visitPlan" style="width: 239px;" value="<?php echo ($visitRecordDetail[visitPlanSubject]); ?>" visitplan_id="<?php echo ($visitRecordDetail[visitPlanId]); ?>" placeholder="从客户拜访计划中选择"/>
            <span class="fa fa-search fa-lg" style="position: absolute;right: 10px;top: 10px;color: #ccc;"></span>
        </li>
    </ul>
    <ul>
        <li class="fll">关联商机</li>
        <li><input style="width: 239px;text-overflow: ellipsis;white-space: nowrap;" type="text" id="SJ" value="<?php echo ($visitRecordDetail[bizSubject]); ?>" biz_id="<?php echo ($visitRecordDetail[bizId]); ?>" disabled="true"  /></li>
    </ul>
    <ul>
        <li class="fll"><span>*</span>拜访对象</li>
        <li style="position: relative">
            <input type="text" id="linkman" style="width: 239px;" value="<?php echo ($visitRecordDetail[cusClientNames]); ?>" title="<?php echo ($visitRecordDetail[cusClientNames]); ?>" linkman_ids="<?php echo ($visitRecordDetail[cusClientIds]); ?>" placeholder="从客户联系人中选择"/>
            <span class="fa fa-search fa-lg" style="position: absolute;right: 10px;top: 10px;color: #ccc;"></span>
        </li>
    </ul>
    <ul>
        ...
        <li>
            <select name="source" id="visitMethod">
                ...
                <option value="1" <?php if($visitRecordDetail[visitMethod] == 1): ?>selected<?php endif; ?> >电话拜访</option>
                <option value="2" <?php if($visitRecordDetail[visitMethod] == 2): ?>selected<?php endif; ?> >现场拜访</option>
                <option value="3" <?php if($visitRecordDetail[visitMethod] == 3): ?>selected<?php endif; ?> >其他</option>
            </select>
        </li>
    </ul>
    <ul>
        <li class="fll"><span>*</span>拜访日期</li>
        <li> <input type="text" class="laydate-icon" id="visitTime" value="<?php echo ($visitRecordDetail[visitRecordDate]); ?>" style="width: 180px;"/></li>
    </ul>
    <ul>
        <li class="fll">交流内容</li>
        <li><textarea style="height: 100px;width: 500px;color: red;" id="exchangesContent" ><?php echo ($visitRecordDetail[exchangesContent]); ?></textarea><span>不超过500个字</span></li>
    </ul>
    <ul>
        <li class="fll">跟进策略</li>
        <li><textarea style="height: 100px;width: 500px;color: red;" id="followTactics" ><?php echo ($visitRecordDetail[followTactics]); ?></textarea><span>不超过500个字</span></li>
    </ul>
</div>
...
<script>
    ...
    function do_save() {
        ...
        var visitRecordId = '<?php echo ($_GET[visitRecordId]); ?>'; //拜访记录id

        var loadIdx = layer.load(0);
        $.post("<?php echo U('Office/saveVisitRecord');?>", {"visitRecordId":visitRecordId, "cusId":cusId, "cusClientIds":cusClientIds, "visitMethod":visitMethod, "bizId":bizId, "followTactics":followTactics, "visitRecordDate":visitTime, "exchangesContent":exchangesContent }, function(data) {
