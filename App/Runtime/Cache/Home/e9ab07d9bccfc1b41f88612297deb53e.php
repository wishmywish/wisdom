...
<body>
    ...
    <div style="text-align: center;">
        <button class="submitBtn" onclick="submit();">确定</button>
        <button class="resetBtn" onclick="reset();">关闭</button>
    </div>
...
<script type="text/javascript" src="/wisdom/Public/static/um/lang/zh-cn/zh-cn.js"></script>
    ...
    <script>
        ...
        function initSelect2() {
            var recorderName = '<?php echo ($recorderName); ?>';
            var recorder = '<?php echo ($recorder); ?>';
            if (recorder == "") {
                return;
            }
            var rs = recorder.split(",");
            var rns = recorderName.split(",");
            for (var i = 0; i < rs.length; i++) {
                var opt = '<option value="' + rs[i] + '">' + rns[i] + '</option>';
                $("#select2").append(opt);
            }
        }
        ...
        function submit() {
            ...
            // 将用户名设定到父画面
            parent.$("#recorderName").val(recorderName);
            // 将用户ID设定到父画面
            parent.$("#recorder").val(recorder);
