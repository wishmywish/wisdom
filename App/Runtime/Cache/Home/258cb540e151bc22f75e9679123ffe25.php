...
<div class="content">
    <table border="0" cellpadding="0" cellspacing="0" id="bizList">
        <tr style="height: 25px;">
            <th width="117" height="25">名称</th>
        </tr>
        <?php if(is_array($bizList)): foreach($bizList as $key=>$vo): ?><tr style="height: 25px;">
            <td>
                <input type="radio" name="more" value="<?php echo ($vo['bizId']); ?>" id="biz<?php echo ($vo[bizId]); ?>" <?php if($vo[bizId] == $_GET[bizId]): ?>checked<?php endif; ?>  />
                <label for="biz<?php echo ($vo[bizId]); ?>"><?php echo ($vo['bizSubject']); ?></label>
            </td>
...
<script>
    ...
    var selected_bizId = '<?php echo ($_GET[bizId]); ?>'; //选中的bizId,初始化值

    //选中商机
    $("#bizList").on("click","input[name=more]",function(){
        selected_bizId = $(this).val();
    });

    //确定提交,并给父窗口赋值
    function do_submit(){
        var selected_bizNmae = $("#biz"+selected_bizId).siblings().text();

        //父窗口赋值
        var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
        parent.$("#SJ").val(selected_bizNmae);
        parent.$("#SJ").attr("title",selected_bizNmae);
    ...
    $(".search").on("click",function(){
        ...
        $.post("<?php echo U('Office/getBizList_ajax');?>", {"page":1, "pageSize":pageSize, "cusId":cusId, "bizSubject":keywords}, function(data) {
    ...
    $("#last_page").on("click",function(){
        ...
        $.post("<?php echo U('Office/getBizList_ajax');?>", {"page":totalPage, "pageSize":pageSize, "cusId":cusId, "bizSubject":keywords}, function(data) {
    ...
    $("#prev_page").on("click",function(){
        ...
        $.post("<?php echo U('Office/getBizList_ajax');?>", {"page":currentPage-1, "pageSize":pageSize, "cusId":cusId, "bizSubject":keywords}, function(data) {
     ...
     */
    function loadBizList(data){

        var html  = '<tr style="height: 25px;">';
            html +=     '<th width="117" height="25">名称</th>';
            html += ' </tr>';
        for( i in data.bizList ){

            var checked = ''; //是否选中
            if( data.bizList[i].bizId == selected_bizId ){  //如果是选中的商机id,则选中状态
                checked = 'checked';
            }

            html += '<tr style="height: 25px;">';
            html +=     '<td>';
            html +=         '<input type="radio" name="more" '+ checked +' value="'+ data.bizList[i].bizId + '" id="biz'+ data.bizList[i].bizId +'" >';
            html +=         '<label for="biz'+ data.bizList[i].bizId +'">' + data.bizList[i].bizSubject + '</label>';
            html +=     '</td>';
            html += '</tr>';
        }
