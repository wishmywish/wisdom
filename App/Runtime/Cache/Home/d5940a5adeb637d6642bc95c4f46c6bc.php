...
<div class="main">
    ...
    <ul>
        ...
        <li style="position: relative">
            <input type="text" id="linkman" style="padding-right: 30px;" linkman_ids=""  placeholder="从客户联系人中选择"/>
    ...
    <ul>
        ...
        <li>
            <select name="source" id="origin">
                ...
                <?php if(is_array($bizOrigin['list'])): foreach($bizOrigin['list'] as $key=>$vo): ?><option value="<?php echo ($vo[dictId]); ?>"><?php echo ($vo['dictName']); ?></option><?php endforeach; endif; ?>
            </select>
        </li>
    </ul>
    <ul>
        <li class="fll"><span>*</span>商机主题</li>
        <li><input type="text" id="title" style="width: 400px;text-overflow: ellipsis;white-space: nowrap;"/><span>不超过50个字</span></li>
    </ul>
    <ul>
    ...
    <ul>
        ...
        <li>
            <select name="busiState" id="bus_state">
                <option value="0">--请选择--</option>
                <?php if(is_array($bizState['list'])): foreach($bizState['list'] as $key=>$vo): ?><option value="<?php echo ($vo[dictId]); ?>"><?php echo ($vo['dictName']); ?></option><?php endforeach; endif; ?>
            </select>
        </li>
    </ul>
    <ul>
...
//        maybe_price = parseFloat(maybe_price).toFixed(2);
        ...
        $.post("<?php echo U('Office/saveBiz');?>", {"cusId":cusId, "cusClientIds":cusClientIds, "bizOrigin":origin, "bizSubject":title, "cusDemand":require, "bizPhase":bus_state, "expectSignDate":signDate, "bizPrice":price, "expectVolume":maybe_price, "remark":memo}, function(data) {
