    ...
    <div class="list">
        <div class="content cur1" style="margin-bottom:20px;">
            <ul class="fl">
                <li>
                    <span>出差内容</span>
                    <textarea style="float: right;height: 100px; width: 480px;" disabled="disabled"><?php echo ($travel_detail["formInfo"]["content"]); ?></textarea>
                </li>
            </ul>
            <ul class="fl">
                <li>
                    <span>出差地点</span>
                    <input type="text" disabled="disabled" value="<?php echo ($travel_detail["formInfo"]["address"]); ?>"/>
                </li>
            </ul>
            <ul class="fl">
                <li>
                    <span>起始时间</span>
                    <input id="startDate" value="<?php echo ($travel_detail["formInfo"]["beginDate"]); ?>" placeholder="起始时间" style="height: 25px;width: 110px;">
                    <span style="width:80px;">终止时间</span>
                    <input id="endDate" value="<?php echo ($travel_detail["formInfo"]["endDate"]); ?>" placeholder="终止时间" style="height: 25px;width: 110px;">
                    <span style="margin-left: 20px;width: 120px;text-align: left;">出差时长：<?php echo ($travel_detail["formInfo"]["duration"]); ?></span>
                </li>
            </ul>
            ...
            <ul class="fl">
                <li>
                    <span>附件</span>
                    <div class="fj">
                    <?php  if($travel_detail['attachList']) { foreach($travel_detail['attachList'] as $value) { echo '<p><a href="javascript:void();" onclick="javascript:doDownload(\''.$value['url'].'\');">'.$value['displayName'].'</a></p><br>'; } } ?>
                    </div>
                </li>
            </ul>

            <?php if($travel_detail['processStatus'] == '2' and $travel_detail['curProcessNodeOrder'] > '1'): ?><ul class="fl">
                <li>
                    <span>上一级审批人</span>&nbsp;<?php echo ($travel_detail["previousProcessNode"]["handlerUserName"]); ?>
                </li>
            </ul>
            <ul class="fl">
                <li>
                    <span>上一级审批意见</span>
                    <textarea style="float: right;height: 60px;width: 480px;" disabled="disabled"><?php echo ($travel_detail["previousProcessNode"]["comment"]); ?></textarea>
                </li>
            </ul><?php endif; ?>
            <?php if($travel_detail['curHandlerUId'] == $userId) {?>
            <ul class="fl">
                <li>
                    <span>审批意见</span>
            ...
            <?php }?>
            <input type="hidden" id="processId" value="<?php echo $travel_detail['processId'];?>">
            <div style="text-align: center;">
                ...
                <?php }?>
                <?php if($travel_detail['createUserId'] == $userId && ($travel_detail['processStatus'] == '1' || $travel_detail['processStatus'] == '2')) {?>
        ...
        <div class="content">
            <table border="0" cellpadding="0" cellspacing="0">
                ...
                <?php if(!empty($travel_detail["processNodeList"])): if(is_array($travel_detail["processNodeList"])): $i = 0; $__LIST__ = $travel_detail["processNodeList"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nodeList): $mod = ($i % 2 );++$i; if($nodeList['processNodeStatus'] == '2') {?>
...
<script>
    ...
    function doSelectUser() {
        ...
        layer.open({
            ...
            content: "<?php echo U('Office/selectTravelUser');?>" + "?processId=" + processId + "&processNodeId=" + processNodeId + "&comment=" + comment + "&operateType=3&page=" + page + "&busiType=4",
        });
    }
    ...
    function processNodeApproval(operateType) {
        ...
        $.post("<?php echo U('Office/processNodeApproval');?>", {"operateType":operateType, "processId":processId, "processNodeId":processNodeId, "comment":comment}, function(data) {
            ...
            if(data.error_code != 'success') {
                ...
                var busiType = 4;
                var page = "<?php echo ($page); ?>";
                var type = parent.$("#hidBTType").val();
                var startDate = parent.$("#hidSDate3").val();
                var endDate = parent.$("#hidEDate3").val();
                var status = parent.$("#hidBTStatus").val();

                parent.$("#businessTripContentList").empty();
                ...
                $.post("<?php echo U('Office/getApprovalList');?>", {"busiType":busiType, "page":page, "type":type, "startDate":startDate, "endDate":endDate, "status":status}, function(data) {
                    ...
                    parent.$("#businessTripContentList").append(content);
                    layer.close(loadIdx);
