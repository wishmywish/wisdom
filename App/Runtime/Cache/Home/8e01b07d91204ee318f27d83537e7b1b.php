    ...
    <div class="list">
        <div class="content cur1">
           <ul class="fl">
               <li>
                   <span>费用内容</span>
                   <textarea style="float: right;height: 100px; width: 480px;" disabled="disabled"><?php echo ($fee_detail["formInfo"]["content"]); ?></textarea>
               </li>
           </ul>
            <ul class="fl">
                <!-- <li>
                ...
                <li>
                    <span>费用类型</span>
                    <input type="text" disabled="disabled" value="<?php echo ($fee_detail["formInfo"]["costType"]); ?>"/>
                    <span>报销金额</span>
                    <input type="text" disabled="disabled" value="<?php echo ($fee_detail["formInfo"]["money"]); ?>"/>
                </li>
            ...
            <ul class="fl">
                <li>
                    <span>附件</span>
                    <div class="fj">
                    <?php  if($fee_detail['attachList']) { foreach($fee_detail['attachList'] as $value) { echo '<p><a href="javascript:void();" onclick="javascript:doDownload(\''.$value['url'].'\');">'.$value['displayName'].'</a></p><br>'; } } ?>
                    </div>
                </li>
            </ul>
            <?php if($fee_detail['processStatus'] == '2' and $fee_detail['curProcessNodeOrder'] > '1'): ?><ul class="fl">
                <li>
                    <span>上一级审批人</span>&nbsp;<?php echo ($fee_detail["previousProcessNode"]["handlerUserName"]); ?>
                </li>
            </ul>
            <ul class="fl">
                <li>
                    <span>上一级审批意见</span>
                    <textarea style="float: right;height: 60px;width: 480px;" disabled="disabled"><?php echo ($fee_detail["previousProcessNode"]["comment"]); ?></textarea>
                </li>
            </ul><?php endif; ?>
        ...
        <div class="content">
            <table border="0" cellpadding="0" cellspacing="0">
                ...
                <?php if(!empty($fee_detail["processNodeList"])): if(is_array($fee_detail["processNodeList"])): $i = 0; $__LIST__ = $fee_detail["processNodeList"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nodeList): $mod = ($i % 2 );++$i; if($nodeList['processNodeStatus'] == '2') {?>
...
<script>
    ...
    function doSelectUser() {
        ...
        layer.open({
            ...
            content: "<?php echo U('Office/selectIndexFeeUser');?>" + "?processId=" + processId + "&processNodeId=" + processNodeId + "&comment=" + comment + "&operateType=3&page=" + page + "&busiType=1",
        });
    }
