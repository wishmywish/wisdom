        ...
        success: function (data) { //成功
            ...
            if (data.error_no === '999999') {
                ...
                if (filetype === 'file') {
                    ...
                    if(showkey=="#show_img_other_7"||showkey=="#show_img_other_6"||showkey=="#show_img_other_5"||showkey=="#show_img_other_4"||showkey=="#show_img_other_1"){
                        var status=showkey.charAt(showkey.length -1);
                        $(showkey).html("<a target='_blank'><img src='" + img + "' width='100' height='100'></a><a style='padding-left:10px;' onclick='delimg("+status+")'>删除</a>");
                    }else{
                        $(showkey).html("<a target='_blank'><img src='" + img + "' width='100' height='100'></a>");
                    }
...
//  var phone = $.trim($('#phone').val());		//
    ...
    var position = $("#position").val();  //员工职位

    //console.log(birthday);return;
   ...
   /* if (!(/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{4}$/).test(idCard)) {
    ...
    //入库
    $.post(APP+"/Api/Api/inport","command=addDeptuser&companyId="+companyId+"&departId="+departId+"&nickName="+nickName+"&trueName="+trueName+"&officePhone="+officePhone+"&otherPhone="+otherPhone+"&email="+email+"&userLevel="+userLevel+"&mobile="+mobile+"&sex="+sex+"&password="+password+"&headLogo="+headLogo+"&fax="+fax+"&homePhone="+homePhone+"&homeAddress="+homeAddress+"&birthday="+birthday+"&invitationCode="+invitationCode+"&position="+position,function(rel){
...
//  var phone = $.trim($('#phone').val());		//
    ...
    if(!sex){  //没选性别
        var fieldkey = "nickName|trueName|email|mobile|fax|homePhone|homeAddress|birthday|invitationCode|companyId|departId";
        var fieldval = nickName+"|"+trueName+"|"+email+"|"+mobile+"|"+fax+"|"+homePhone+"|"+homeAddress+"|"+birthday+"|"+invitationCode+"|"+companyId+"|"+departId;
    }else{
        var fieldkey = "nickName|trueName|email|mobile|sex|fax|homePhone|homeAddress|birthday|invitationCode|companyId|departId";
        var fieldval = nickName+"|"+trueName+"|"+email+"|"+mobile+"|"+sex+"|"+fax+"|"+homePhone+"|"+homeAddress+"|"+birthday+"|"+invitationCode+"|"+companyId+"|"+departId;
    }
...
//  $.post(APP+"/Api/Info/conf","command=modiUserInfo&companyId="+companyId+"&departId="+departId+"&nickName="+nickName+"&trueName="+trueName+"&officePhone="+officePhone+"&otherPhone="+otherPhone+"&email="+email+"&userLevel="+userLevel+"&mobile="+mobile+"&sex="+sex+"&password="+password+"&headLogo="+headLogo+"&fax="+fax+"&homePhone="+homePhone+"&homeAddress="+homeAddress+"&birthday="+birthday+"&invitationCode="+invitationCode,function(rel){
    $.post(APP+"/Api/Api/inport","command=modiUserInfo&userid="+userId+"&fieldkey="+fieldkey+"&fieldval="+fieldval,function(result){
		console.log(result);
...
function  role_delete(id,i) {
    $.post(APP + "/Api/Web/role_del", "f_id=" + id, function (rel) {
        ...
        } else if(rel.code=="isSuper") {
            layer.msg("超级管理员不能删除");
        }else {
            layer.msg("角色不存在,删除失败");
        }
    }, "json");
}
...
function returnHome(){
    ...
    window.location.href= APP+"/Home/Office/index";
    //window.location.href= APP+"/Home/Business/index";
}
