...
(function($) {
	...
	var F = function(z, A) {
		...
		function buildTree(a) {
			...
			$(C.selector + " .easytree-icon").on("click", a, toggleNodeEvt);
			$(C.selector + " .easytree-title").on("click", a, toggleNodeEvt);
			var l = new Date();
			if (B.enableDnd) {
		...
		function getIconHtml(a) {
			...
			if (B.checkIcons) {
				return '<span><input type="checkbox" class="easyChk" value="' + a.deptId + '" id="chk' + a.id + '" rel="' + a.text + '" onclick="' + B.checkFunction + '" /></span>';
			}
			if (B.radioIcons) {
				if(a.lastFlag == 1) {
					return '<span><input type="radio" name="radio" class="easyChk" value="' + a.deptId + '" id="chk' + a.id + '" rel="' + a.text + '" onclick="' + B.checkFunction + '" /></span>';
				} else {
					return '<span></span>';
				}
			}
