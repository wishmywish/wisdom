...
//                $p = M('tradetype')->where('f_tradeID='.$tradeid)->find();
    ...
    // var_dump($this->out_arr['linkOne']);
    // var_dump($this->out_arr['linkTwo']);
    // exit();
            if (isset($this->out_arr['authorization'])) {
                $this->out_arr['result']="000000";                 
            }else{
...
//         echo M('surveyanswer')->getLastSql();
...
//         echo M('surveyanswerdetail')->getLastSql();exit;
        // echo  $list->getLastSql();
         foreach($list as $k=>$v){
             if($v["f_type"]==4){//解析图片数据
                 $answer=explode(",",$v["f_answer"]);
//                 echo count($answer)."答案长度";
//                 exit;
                 foreach($answer as $j=>$vo){
                     $r=M("files")->where("f_id=".$vo)->find();
//                     echo M("files")->getLastSql();
//                     echo $r["f_filepath"];
//                     echo $r["f_filename"];
...
//       pt($idarray);exit;
    ...
    public function getAccessList(){
        ...
        $this->out_arr['list'] = M("sysaccessfield")->page($page.','.$pageSize)->select();
...
//       exit;
       ...
       if($type==1){
           ...
           foreach ($list1 as $key => $val) {
                 ...
                 $unReadMessage = A('Info')->getTalkNum($unReadMessage_dealerid); //获取未读信息数
                 $userid=$val['f_userid'];
...
// exit();
    ...
    function  changeStatutask(){
        ...
        $re=M("task")-> where($map)->setField('f_status',$status);
		if($re==1){
    ...
    public function addAdmin(){
        $this->getConf();//取数据
        // var_dump($this->in_arr['accessname']);
        // var_dump($this->in_arr['accessvalue']);
        // exit();
        $map['accessname'] = $this->in_arr['accessname'];
        $map['accessvalue'] = $this->in_arr['accessvalue'];
        $re=M('sysaccess')->add($map);
        if($re){
        $this->out_arr['result'] = '000000';
        }
