...
class UcpaasController extends Controller {
    ...
    private  $SoftVersion_meeting = '2015-06-30';
    ...
    function in_ucpaas($option,$body,$method='post'){
        ...
        if($option_arr[0]=="Conference"){
            $url = $this->Ucpaas."/".$this->SoftVersion_meeting."/Accounts/".$this->accountSid."/".$option."/?sig=".$this->SigParameter();//发送的URL
        }else{
