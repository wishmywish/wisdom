...
//        exit;
        if ($tasktypeid == 1) { //推广
            ...
            $reModi['f_taskrequirements'] = ereg_replace("\n", "", $reModi['f_taskrequirements']);
            $total = 2 * $reModi["f_eachcoin"] * $reModi["f_totalcopies"];
            $reModi['aduit_list'] = M('tasksuperaddress')->where('f_task_id=' . $taskid)->select();
            ...
            if (isset($content)) {
                ...
                $reModi['attachEight'] = M("files")->where("f_id=" . $content["f_fileid8"])->find();
                ...
                $reModi['linkSeven'] = __ROOT__ . '/Public/upfile/' . $reModi['attachSeven']['f_filepath'] . $reModi['attachSeven']['f_filename'];
                $reModi['linkEight'] = __ROOT__ . '/Public/upfile/' . $reModi['attachEight']['f_filepath'] . $reModi['attachEight']['f_filename'];
            }
        }
        ...
        } elseif ($tasktypeid == 7) { //稽核赚
            $this->assign("total", $total);
            $this->display("Promotion:auditPreview");
        }
...
//        supervisionUser
//        supervisonUser
        $url = "&credit=" . $bao_money . "&supervisionUser=" . $jianguan;
