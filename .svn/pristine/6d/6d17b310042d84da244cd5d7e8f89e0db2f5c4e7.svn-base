...
define('APPKEY','argmSmuc3d5YF552H8aFH9');
define('APPID','ska0mRXuTn5jhc2xptbql5');
define('MASTERSECRET','jjuOs9Tf98ATFCEno6wM83');
define('HOST','http://sdk.open.api.igexin.com/apiex.htm');
define('CID','6ad54f4a91b61ad551600439cf7eb197');
//define('DEVICETOKEN','');
//define('Alias','小宝招商');
...
$task_id = trim(isset($_POST['task_id'])?$_POST['task_id']:"");
$type = $_GET['type'];
if($type==1){
    //全推
    pushMessageToApp();
...
function IGtLinkTemplateDemo(){
        ...
        $template ->set_appId(APPID);//应用appid
        $template ->set_appkey(APPKEY);//应用appkey
        $template ->set_title("哈哈");//通知栏标题
        $template ->set_text("呵呵");//通知栏内容
...
function IGtNotificationTemplateDemo(){
        global $title,$text,$taskid;
        $template =  new IGtNotificationTemplate();
        $template->set_appId(APPID);//应用appid
        $template->set_appkey(APPKEY);//应用appkey
        $template->set_transmissionType(1);//透传消息类型
        $template->set_transmissionContent($taskid);//透传内容
        $template->set_title($title);//通知栏标题
        $template->set_text($text);//通知栏内容
        $template->set_logo("http://wisdom.51lick.com/Public/upfile/logo/logo.png");//通知栏logo
        $template->set_isRing(true);//是否响铃
        $template->set_isVibrate(true);//是否震动
        $template->set_isClearable(true);//通知栏是否可清除
        //$template->set_duration(BEGINTIME,ENDTIME); //设置ANDROID客户端在此时间区间内展示消息
