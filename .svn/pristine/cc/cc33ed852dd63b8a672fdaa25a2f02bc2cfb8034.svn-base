...
class UserInfoController extends Controller {
    ...
    public function index(){
        ...
        A("Api/Fun")->isAccess($access);//ÅĞ¶Ï²Ëµ¥È¨ÏŞ
        $this->assign('access',$access);//´«ÈëHTML£¬¹¦ÄÜÈ¨ÏŞÊ¹ÓÃ
    ...
    public function userAccountUpdate(){
        ...
        if($m){
            $re['result']="000000";
            $re_1=M('useraccount')->where($m)->save($map);
        }
        
        echo json_encode($re);
        // $this->A('Api/Web')->pushConf();
    }

    public function redPacketCount(){
        set_theme();
        A("Taskadmin/Fun")->islogin();//¼ì²âÊÇ·ñµÇÂ¼
        $re=M('newuserclaim')->count();
        $re_1=count(M('newuserclaim')->group('f_userid')->select());
        // var_dump($re_1);
        // var_dump($re);exit();
        $this->assign("re",$re);
        $this->assign("re_1",$re_1);
